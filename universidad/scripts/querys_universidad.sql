...
SELECT p.nombre, p.apellido1, p.apellido2 FROM persona p LEFT JOIN asignatura a ON p.id = a.id_profesor WHERE a.id IS NULL AND p.tipo = "profesor";
...
SELECT d.nombre AS departamento_nombre FROM departamento d LEFT JOIN profesor p ON p.id_departamento = d.id WHERE COUNT()
